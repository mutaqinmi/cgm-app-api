...
export default function Drawer(props: {className?: string; children: React.ReactNode; setShowDrawer: (showDrawer: boolean) => void}) {
  return <div>
    <div className="w-screen h-screen bg-black z-40 fixed top-0 bg-opacity-50 md:hidden" onClick={() => props.setShowDrawer(false)}></div>
    <div className={`w-80 h-screen shadow-md p-7 bg-white fixed top-0 z-50 md:hidden ${props.className}`}>
      <div className="flex items-center w-full mb-12 ml-[-6px] md:hidden">
