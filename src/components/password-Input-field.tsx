...
export default function PasswordInputField(props: {password: string; setPassword: (password: string) => void}){
    ...
    return <div className="flex relative my-2">
        <input required id="password" type={show ? "text" : "password"} className="peer w-full outline-none px-4 py-3 text-base rounded-md bg-white border-2" onChange={(event) => props.setPassword(event.currentTarget.value)} value={props.password}/>
